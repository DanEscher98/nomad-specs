...
\begin{equation}
(\mathit{init\_key}, \mathit{resp\_key}) = \text{HKDF}(\mathit{hash}, \text{``nomad v1 session keys''}, 64)
\end{equation}

The initiator uses $\mathit{init\_key}$ for sending and $\mathit{resp\_key}$ for receiving; the responder uses the inverse.
...
\begin{itemize}
    \item Idempotence: $\mathit{apply}(s, d) = \mathit{apply}(\mathit{apply}(s, d), d)$
    \item Convergence: arbitrary message orderings reach same state
    \item Replay rejection: repeated nonces are detected
    \item Nonce uniqueness: no collisions within epoch
\end{itemize}

\subsection{Reference Implementation}

A Rust implementation of \nomad{} has passed the complete conformance suite including all unit, protocol, and adversarial tests. The implementation is available as open source.
...
The protocol specifications, conformance test suite, and reference implementation are available as open source at \url{https://github.com/[redacted]/nomad-specs}.
